...
public:
    int destroyTargets(vector<int> &nums, int space)
    {
        ...
        unordered_map<int, pair<int, int>> uipii;
        for (int i = 0; i < nums.size(); i++)
        {
            int cur = nums[i];
            int cup = cur % space;
            if (uipii.count(cup) < 1)
            {
                uipii[cup] = {cur, 1};
            }
            else
        ...
        int result = 0;
        int max_cnt = 0;
        for (pair<int, pair<int, int>> pipii : uipii)
        {
            if (pipii.second.second > max_cnt)
            {
                result = pipii.second.first;
                max_cnt = pipii.second.second;
            }
            else if (pipii.second.second == max_cnt && pipii.second.first < result)
            {
                result = pipii.second.first;
            }
        }
