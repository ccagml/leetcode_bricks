...
{
public:
    unordered_map<int, unordered_map<int, int>> uipii;
    Graph(int n, vector<vector<int>> &edges)
    {
        for (int i = 0; i < edges.size(); i++)
        {
            uipii[edges[i][0]].insert(std::make_pair(edges[i][1], edges[i][2]));
        }
    }

    void addEdge(vector<int> edge)
    {
        uipii[edge[0]].insert(std::make_pair(edge[1], edge[2]));
    }
    ...
    int shortestPath(int node1, int node2)
    {
        ...
        for (pair<int, int> pipii : uipii[node1])
        {
            qii.push(pipii);
            uii[pipii.first] = pipii.second;
        }
        while (qii.size() > 0)
        {
            pair<int, int> temp = qii.front();
            qii.pop();
            if (temp.first == node2)
            {
                min_result = min(min_result, temp.second);
            }
            else
            {
                for (pair<int, int> pipii : uipii[temp.first])
                {

                    int sb = pipii.second + temp.second;
                    if (min_result > sb && (uii.count(pipii.first) == 0 || uii[pipii.first] > sb))
                    {
                        uii[pipii.first] = sb;
                        qii.push({pipii.first, sb});
                    }
                }
            }
        }

        return min_result == 99999999 ? -1 : min_result;
    }
};
