...
struct compare
{
    bool operator()(const std::pair<int, string> &t1, const std::pair<int, string> &t2)
    {
        // true使得t1排后面?
...
public:
    ...
    unordered_map<string, priority_queue<pair<int, string>, vector<pair<int, string>>, compare>> cus_s;
    ...
    FoodRatings(vector<string> &foods, vector<string> &cuisines, vector<int> &ratings)
    {
        for (int i = 0; i < foods.size(); i++)
        {
            ...
            cus_s[cuisines[i]].push({ratings[i], foods[i]});
        }
        ...
        //     {
        //         std::cout << foods[i] << "<" << foods[j] << "=" << (foods[i] < foods[j]) << std::endl;
        //     }
        // }
    }
    ...
    void changeRating(string food, int newRating)
    {
        ...
        cus_s[food_cus[food]].push({newRating, food});
    }

    string highestRated(string cuisine)
    {
        while (cus_s[cuisine].top().first != food_rat[cus_s[cuisine].top().second])
        {
            cus_s[cuisine].pop();
        }
        return cus_s[cuisine].top().second;
    }
};
